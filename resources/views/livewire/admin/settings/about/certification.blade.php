    ...
    @foreach ($locales as $locale => $name)
      <flux:tab.panel class="space-y-4" name="{{ $locale }}">
        <flux:input
          badge="Requerido"
          label="Título"
          wire:model="about.{{ $locale }}.certification.title"
        />

        <flux:editor
          badge="Requerido"
          label="Descripción"
          wire:model="about.{{ $locale }}.certification.description"
        />

        <flux:file-upload label="Imagen Principal" wire:model="newCertificationMainImage">
          <flux:file-upload.dropzone
            inline
            text="300x500 - JPG, PNG, SVG hasta 2MB"
            with-progress
          />
        </flux:file-upload>
        ...
        @if (isset($about['certification_main_image']))
          <div class="mt-4 flex flex-col gap-2">
            <flux:file-item
              heading="{{ Storage::disk('public')->url($about['certification_main_image']) }}"
        ...
        @if (isset($about['certification_secondary_image']))
          <div class="mt-4 flex flex-col gap-2">
            <flux:file-item
              heading="{{ Storage::disk('public')->url($about['certification_secondary_image']) }}"
