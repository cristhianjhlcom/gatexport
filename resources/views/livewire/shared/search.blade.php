<flux:navbar>
  ...
  <flux:modal class="!radius-sm max-w-120 overflow-hidden! p-2! my-[12vh] max-h-screen w-full space-y-2 bg-white" name="search"
    style="border-radius: 0.5rem !important;" variant="bare">
    <flux:input icon="magnifying-glass" placeholder="{{ __('layouts.navigation.search') }}..." wire:model.live.debounce.500ms="search" />
    @if ($this->search !== '')
      <ul>
        @forelse($this->products as $product)
          <li :key="$product->id">
            <flux:button class="flex w-full items-center justify-start gap-x-4" href="{{ $product->showUrl }}" variant="ghost">
